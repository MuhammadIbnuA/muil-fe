  ...
  return (
    <div style={{ textAlign: "center" }}>
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: "2rem" }}>
        ...
        <input type="text" value={harga} onChange={(e) => setHarga(e.target.value)} placeholder="Kategori" style={{ marginRight: "1rem" }} />
      ...
      <table style={{ margin: "0 auto", borderCollapse: "collapse", width: "100%" }}>
        <thead>
          <tr>
            ...
            <th style={{ padding: "0.5rem", backgroundColor: "LightCyan" }}>Kategori</th>
            <th style={{ padding: "0.5rem", backgroundColor: "LightCyan" }}>Waktu Kedatangan</th>
...
async function createMenu(menuData) {
  try {
    ...
    const response = await fetch("https://muil-be.vercel.app/menu", {
      method: "POST",
      headers: {
...
async function updateMenu(menuId, updateData) {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(`https://muil-be.vercel.app/menu/${menuId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
